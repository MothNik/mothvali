...
def _convert_to_validated_type(
    ...
    Returns
    -------
    converted_value : object of ``output_type``
        The converted value.
...
def _convert_to_bound_validated_value(
    value: _ValueType,
    name: str,
    ...
    # the comparison is performed and the value is either returned, clipped, or an
    # error is raised
    if comparison_operator(value, bound):
        return value
...
def _convert_to_validated_scalar(
    ...
    Returns
    -------
    validated_value : object of type ``output_type``
        The validated value.
    ...
    for bound, bound_kind, is_inclusive in [
        ...
        value = _convert_to_bound_validated_value(
            value=value,
            name=name,
            bound=bound,
...
def isinstance_incl_none(
    ...
    value: any
        The value to check.
    types : type or (type or ``None``, ...) or ``None``
    ...
    is_instance : :obj:`bool`
        Whether the value is an instance of one of the provided types (``True``) or not
        (``False``).
