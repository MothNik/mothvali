...
This module provides input validations for array-like data structures.
"""
...
from .scalars import RealNumeric

# === Typing ===

_NumpyDType = TypeVar("_NumpyDType", bound=np.generic)
...
@overload
def convert_to_validated_numeric_nd_array_like(
    value: Any,
    name: str,
    dim: Literal[1],
    shape_limits: List[Tuple[Optional[int], Optional[int]]],
    allowed_from_dtype_kinds: NumPyDTypeKinds,
    enforce_finite: bool,
    output_dtype: Type[_NumpyDType],
) -> np.ndarray[Tuple[int], np.dtype[_NumpyDType]]: ...
...
def convert_to_validated_numeric_nd_array_like(
    ...
    enforce_finite: bool,
    output_dtype: Type[_NumpyDType],
) -> np.ndarray[Tuple[int, int], np.dtype[_NumpyDType]]: ...
...
def convert_to_validated_numeric_nd_array_like(
    ...
    allowed_from_dtype_kinds : :obj:`NumPyDTypeKinds`
        The valid data type kinds that ``value`` is allowed to be converted from.
    enforce_finite : :obj:`bool`
        Whether to enforce that all entries are finite, i.e., not ``nan``, ``-inf``, or
        ``inf`` (``True``) or not (``False``).
    output_dtype : a ``numpy.dtype``
        The data type of the output NumPy Array.
        If required, the data type conversion is done with
        ``value.astype(output_dtype, casting="safe")``.
    ...
    if value_array.dtype.kind not in allowed_from_dtype_kinds.value:
        raise TypeError(
            f"Expected '{name}' to be a {dim}D Array-like of "
            f"{allowed_from_dtype_kinds.to_error_message_str()} values, but got a "
            f"{dim}D Array-like with dtype '{value_array.dtype}'."
        )
    ...
    if output_dtype == value_array.dtype.type:
        return value_array

    try:
        return value_array.astype(output_dtype, casting="safe")
...
def convert_to_validated_real_numeric_1d_array_like(
    value: Any,
    name: str,
    min_size: Optional[int] = None,
    max_size: Optional[int] = None,
    enforce_finite: bool = False,
    output_dtype: Type[_NumpyDType] = np.float64,
) -> np.ndarray[Tuple[int], np.dtype[_NumpyDType]]:
    """
    ...
    enforce_finite : :obj:`bool`, default=``False``
        Whether to enforce that all entries are finite, i.e., not ``nan``, ``-inf``, or
        ``inf`` (``True``) or not (``False``).
    output_dtype : a ``numpy.dtype``, default=``numpy.float64``
        The data type of the output NumPy Array.
        If required, the data type conversion is done with
        ``value.astype(output_dtype, casting="safe")``.
    ...
    return convert_to_validated_numeric_nd_array_like(
        value=value,
        name=name,
        dim=1,
        shape_limits=[
        ...
        allowed_from_dtype_kinds=NumPyDTypeKinds.REAL_NUMERIC_NO_BOOL,
        enforce_finite=enforce_finite,
        output_dtype=output_dtype,
    )


def convert_to_validated_real_numeric_2d_array_like(
    value: Any,
    name: str,
